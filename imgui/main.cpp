...
public:
    ...
    bool Load(SDL_Surface* surface, SDL_Renderer* render) {
        if (!surface||!render) return false;
        // 获取表面信息
        width = surface->w;
        height = surface->h;
        // 转换为纹理
        SDL_Texture* texture = SDL_CreateTextureFromSurface(render, surface);
        textureID = (ImTextureID)(intptr_t)texture;
    }
...
int main(int, char**)
{
    ...
    if (SDL_Init(SDL_INIT_VIDEO | SDL_INIT_GAMEPAD | SDL_INIT_CAMERA | SDL_INIT_AUDIO) < 0)
    {
